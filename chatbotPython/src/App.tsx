...
import { formatMessageText, getWebsiteInfo } from './utils/websiteInfo';
  ...
  const renderMessageContent = (text: string) => {
    ...
    return parts.map((part, index) => {
      if (part.match(urlRegex)) {
        const websiteInfo = getWebsiteInfo(part);
        if (websiteInfo) {
          return (
            <span key={index} className="inline-flex items-center gap-2 bg-gray-600/50 rounded-full px-3 py-1 text-sm hover:bg-gray-600 transition-colors">
              <img 
              ...
              />
              <span className="truncate">{websiteInfo.name}</span>
            </span>
          );
        }
        return (
